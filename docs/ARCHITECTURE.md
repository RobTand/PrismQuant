...
was open, then landed contract v21 (`b8b1cb38`, its #313) before it merged,
...
rule admits them. Consequence on the pinned (v21) table: all ten cells admit
on their own evidence — the eight dense cells exactly as at v17, the two
routed-MoE cells for the first time, by a Tessera measurement and not by a
predicate change here; the four dense E4M3 cells now
...
the serving pin JSON and its module constants all name `b8b1cb38…` /
`34f1da79…` (contract v21, lane schema v8) in one commit, and
...
same ten cell ids in the same order. Tessera master at the re-pin (`8446227d`,
nine merges on) packaged byte-identical contract bytes; no tag names the pinned
...
SCOPE (§5.7): the v8 table is scoped and the operator supplies the scope
...
gate admits them, Tessera #313). Folding them into the dev pin would make an export-lane edit
...
`34f1da79…2a2c3e` (Tessera master `b8b1cb38…`, contract v21; the release
...
gate, including that the two `routed_moe` cells are refused and every dense cell
is not. `prismaquant/tessera_runtime/README.md` carries the exact edit that
...
Tessera `b8b1cb38d581721d084860db1ae4a25b6afae50f` (master, the merge of
Tessera #313; version `0.1.0`, contract v21, lane schema v8; the release
`e78959ed…` carried v20; first pinned 2026-09-04 at `5acc2a6f…`, contract v17)
