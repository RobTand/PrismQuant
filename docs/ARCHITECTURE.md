...
§9.2.1). Re-stamped (2026-08-29, `codex/arch84-profile-table-pin`, landed on main as
#101 after the sweep stamped below) for the **§8.4 model-profile
conformance pin**: the matrix is now parsed in CI and checked in **both
directions** against the Python profile registry, model-structure specs and
serving-profile specs — a newly registered profile missing from the matrix and
a stale row whose profile was removed are each a failing test. The `glm5_next`
drift recorded in the stamp immediately below, and its whole class, is
therefore now caught by a gate rather than by a hand sweep. Re-stamped (2026-08-29,
