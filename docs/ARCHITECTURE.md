...
As of: 2026-08-31 · `muse/wo-c-trellis-export-20260831` — stamps
follow, newest first, each recording its own source branch and date. Re-stamped
(2026-08-31, `muse/wo-c-trellis-export-20260831`) for the **Gridbook trellis export into the `gridbook` container** (§6, §9, `docs/design/trellis_export_2026-08-31.md`). `prismaquant/export_nvfp4_cb.py` now emits `TCQ_E*M*_R*` units beside NVFP4-CB/FP8-CB/stock: per Linear `<target>.wire_bytes` (opaque `gridbook.trellis.wire.v1` blob, the only carrier of schedule/alphabets/offsets/scale plane) and E2M1's `trellis_input_global_scale` (`e2m1_group16_ue4m3_static`, derived from calibration activations via `nvfp4_activation_contract.input_global_scale_from_max_abs`, fail-closed on missing). Fused `q/k/v` and `gate/up` are refused by name (per-role wires cannot be concatenated; `gridbook/config.py` is the authority, reuse the same union-find grouping). Dense-only; routed MoE with a trellis rung fails naming the missing `lane_eligibility` cell. Per-artifact route gating (principle 9, `gridbook.lane-eligibility.v3`): every trellis unit's `rate_q256` must appear in a `sm_121` `dense` cell's `rungs_q256` (E2M1 `[512]` only vs `candidate_rungs_q256` `[384,512,640,768,896]` — a candidate may have no cell, export gates on the cell). `backed_with_serve_flag` records `GRIDBOOK_TRELLIS_E*M*=1` + `GRIDBOOK_TRELLIS_E*M*_MODE=resident|streamed` in `selection_serving_lane_provenance` and the shipcard as structured fields. The wire bytes come from `ProductionWeightCache` (WO-B seam; do not re-encode at export, fail closed if missing). `prismaquant/layer_config.py` now canonicalizes `TCQ_*`, `serving_profile_specs/nvfp4_cb.json` admits `ALL_LEGAL_TRELLIS_FORMAT_NAMES`, `quantization_config.format` is `mixed-precision` when families mix. Golden vectors under `tests/fixtures/trellis_wire_golden/` (both layouts, both families, non-multiple-256 cols, non-multiple-16 rows, 4 E2M1 rungs) plus `tests/test_trellis_wire_golden.py` and `tools/verify_trellis_golden_gridbook.py` prove cross-repo agreement without vendoring. `tests/test_trellis_export_smoke.py` builds a 2-layer hidden-256 model with `o_proj`/`down_proj` on `TCQ_E2M1_R512` and asserts the emitted `config.json` + tensor names field-for-field vs `tools/make_trellis_smoke_checkpoint.py`. Re-stamped
