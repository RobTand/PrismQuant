...
minutes to hours long; enabling an MPS daemon adds lifecycle and fault-domain
...
underfilled processes can improve work per joule, evaluate MPS as a compute
throughput A/B, independently of memory admission.  **Adoption cost: medium.**
...
([GPU time-slicing documentation](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/22.9.2/gpu-sharing.html)).
...
memory limits or isolate processes by default; another mechanism such as HAMi
is required
...
gang eviction phases; priority governs lower-priority preemption, and its shard
configuration supports minimum runtimes before preemption/reclaim
...
Run:ai documents that fractional GPUs use NVIDIA time slicing by default and
are placed from requested GPU portions or memory
([Run:ai GPU fractions](https://run-ai-docs.nvidia.com/self-hosted/platform-management/runai-scheduler/resource-optimization/fractions)).

**Inference for PrismaQuant.**  Soft GPU-memory requests based on a discrete
device-memory model are the wrong resource contract for a UMA pool whose
reliable ceiling is system MemAvailable.  KAI's minimum-runtime, priority, and
reclaim ordering are useful policy precedents; its Kubernetes control plane,
reservation pods, and unenforced GPU-memory fractions are not.  Do not adopt
KAI/Run:ai on two Sparks.  **Adoption cost: very high.**
...
install Kueue.  **Adoption cost for the policy: medium; for Kubernetes: high.**
...
Its node memory monitor kills one worker at a time after a usage threshold.  It
prefers retriable work, chooses from the caller with the most running tasks,
kills the latest-started task within that caller, and fails rather than
indefinitely killing the caller's last task
([Ray OOM prevention](https://docs.ray.io/en/latest/ray-core/scheduling/ray-oom-prevention.html)).

**Inference for PrismaQuant.**  Ray's OOM monitor is the closest prior art to
the current governor, but it is level-triggered and its object-store/runtime
machinery is unnecessary.  More importantly, the local project history records
a Ray monitor killing healthy ranks on this UMA platform.  Borrow its
restartability-first and “one task must make progress” rules; do not adopt Ray
as the worker runtime.  **Adoption cost for the rules: low to medium; for Ray:
high.**
...
before/after in-process and Netdata evidence.  **Adoption cost: medium.**
...
Ray kills the latest-started task within its chosen caller
([Ray OOM prevention](https://docs.ray.io/en/latest/ray-core/scheduling/ray-oom-prevention.html)).
...
do not replace real fields with guessed precision.  **Adoption cost: medium to
high.**
...
1. Designate one running job as the protected progress anchor (normally the
   highest-priority/oldest incumbent).
...
recreating the same losing state.  The anchor adapts Ray's documented rule
that its OOM monitor will not indefinitely kill the last task of a caller
([Ray OOM prevention](https://docs.ray.io/en/latest/ray-core/scheduling/ray-oom-prevention.html));
the no-good record converts an eviction into scheduler knowledge; the
one-token limit makes the unknown appetite observable before another unknown
starts.  Exponential backoff can remain for transient errors, but it is no
longer the memory-fit mechanism.
