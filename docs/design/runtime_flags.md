...
| `PRISMAQUANT_TRELLIS_SURFACE` | unset (off) — **RESEARCH** | `trellis_menu.py` (`TRELLIS_SURFACE_ENV`), consumed in `allocator_candidates.build_candidates` | Names a JSON manifest of measured trellis anchors. Unset, `augment_candidates` returns its input object unchanged and the run is byte-identical to one built before the seam existed. **Set, it refuses**, listing every entry of `trellis_menu.UNWIRED_LINKS`: eight links between a built trellis menu and a shipped assignment do not exist. Enabling the surface means landing those with behaviour tests and deleting the refusal, not passing this flag. See `docs/ARCHITECTURE.md` §4.9. |
| `PRISMAQUANT_TRELLIS_ANCHOR_DW` | unset (off) — **RESEARCH** | `trellis_anchor_dw.py` (`TRELLIS_ANCHOR_DW_ENV`), read only by `open_trellis_anchor_source` | Names a store of trellis renders produced by an offline encode in the pinned Stage-6 container, for replay as `compute_aura_cost_streamed(anchor_renderer=...)` so a trellis anchor can be priced in AURA `predicted_dloss` instead of the ladder's weighted SSE. Unset returns `None`; **nothing in `run-pipeline.sh` calls the opener**, so the default path is byte-identical either way. The store holds `dW` only: it declares no cost currency (that belongs to the AURA run) and no runtime contract (§ P14). It produces no anchor by itself — the offline encode driver does not exist yet. See `docs/ARCHITECTURE.md` §4.10. |
