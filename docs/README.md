...
|---|---|---|
| `design/artifact_collections.md` | Content-addressed, format-agnostic control plane for probe-once/export-many collections: typed source/probe/cost/solve/export/evidence/qualification/market/release records, closed-graph reconciliation, and a legacy Qwen artifact census. | CURRENT typed offline control plane — strict schemas, verifier CLI, and tests ship; live pipeline producer adapters and physical qualification remain open |
...
| `design/trellis_rate_surface.md` | Research-only continuous Trellis rate contract: lazy immutable all-legal/dense q256 generation, canonical scale-inclusive footprint validation and honest pre-render recipe identities, same-unit/profile gates, centered finite uncertainty arithmetic, per-tensor compressed RD hulls with cached exact-rational O(log H) lambda lookup, compact off-hull-safe adaptive refinement, and uncertified local/complete measured-surface menus requiring a later exact-byte filter/certificate. | CURRENT research contract — no registry, producer, exporter, pipeline, solver certificate, compiled-kernel, rendered-wire identity, or public-format authority |
...
| Path | What it is | Status |
|---|---|---|
| `lanes/nvfp4-cb/STANDARDS.md` | The producer lane's normative format/kernel/cost contract: NVFP4 reader/interim producer K12..K24, FP8 reader K28..K48 and producer K40/K44/K48, scale coding, kernel standards, the split-aware floored RD cost law, and the served-A/B rule. Runtime behavior is owned by the external Gridbook repository and its packaged contract. | CURRENT — production exports default to two-tier/v2; v1 is explicit legacy read compatibility; the sealed broad SM120 candidate pin is deliberately incompatible pending a new Gridbook package identity |
