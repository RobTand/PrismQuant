...
|---|---|---|
| `design/prismabuild.md` | Deterministic distributed campaign contract: content-bound action keys, immutable CAS receipts, explicit SLURM placement, and optional Dagster assets whose edges bind exact upstream output digests. | CURRENT implementation contract — dependency-free core, SLURM transport, and optional Dagster definition/runner layer are built and CPU-tested; no SLURM or Dagster services are live-deployed and no pipeline default depends on them |
| `design/qtip_native_nvfp4.md` | Research contract for separating QTIP's BlockLDLQ, incoherence transforms, and stateful source coding while preserving a stock NVFP4 control and Gridbook's native W4A4 tensor-core endpoint. Includes the exact four-bit support boundary and scale-plane capacity argument. | CURRENT research design — one-Linear and Gridbook transform arms are implementation/measurement work; no new production format, serving qualification, or EXL3 dependency |
| `results/qtip_trellis_online_hadamard_producer_status_2026-08-30.md` | Exact status of the combined QTIP-basis/E2M1-trellis one-Linear producer seam: transform/Hessian algebra and Gridbook metadata are executable and cross-runtime pinned, while physical wire publication refuses because PrismaQuant has no repository-owned encoder+packer+decoder API. | CURRENT research status — fail-closed scaffold only; no format registration, runtime-pin change, or physical wire round trip |
...
| `design/gridbook_lane_eligibility_contract.md` | The `lane_eligibility` table Gridbook must package inside `runtime_contract.json` so CB serving-lane route status becomes attested rather than asserted (campaign R3, principles 9 and 14): the schema, the closed set of structural facts a predicate may name, and the four rules the DSv4 per-role case needs. Records what Gridbook 0.8.10 actually publishes and why none of it answers the lane question. | SUPERSEDED as a specification (2026-08-30) — Gridbook published a table at `30287aa` (contract v12, schema `gridbook.lane-eligibility.v3`) in a different shape; kept as the record of the ask. PrismaQuant's parser reads the published v3 (ARCHITECTURE.md §9.2.1); CB lanes still resolve `unattested` because the pins remain on 0.8.11, whose contract carries no table |
