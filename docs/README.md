...
|---|---|---|
| `design/prismabuild.md` | Deterministic distributed campaign contract: content-bound action keys, immutable CAS receipts, explicit SLURM placement, and optional Dagster assets whose edges bind exact upstream output digests. | CURRENT implementation contract — dependency-free core, SLURM transport, and optional Dagster definition/runner layer are built and CPU-tested with fake scheduler/in-process paths; no SLURM, Dagster, shared CAS, or telemetry services are live-deployed; replay economics, speculative routing, and OOM isolation are not live-validated; no pipeline default depends on PrismaBuild |
| `design/qtip_native_nvfp4.md` | Research contract for separating QTIP-derived BlockLDLQ, incoherence transforms, and PrismaQuant/Gridbook's existing trellis carrier while preserving a stock NVFP4 control. Includes the exact four-bit support boundary and scale-plane capacity argument. | CURRENT research design — stock-native Arm C is implemented; an external Gridbook transform reference is implemented but unpinned; the combined rotated-trellis producer is absent; QTIP's wire is excluded and distinct; Quartet II is literature-only and EXL3 is source-reading-only; no new production format or serving qualification |
