...
def test_the_ladders_own_currency_is_the_surviving_refusal(tmp_path):
    """The currency entry is a refusal rather than missing plumbing.
    ...
    assert "trellis_rate_surface.py:43-52" in message
...
# ---------------------------------------------------------------------------
# Link 1: no TCQ spec supplies BYTES, on purpose -- they ride the stats instead
# ---------------------------------------------------------------------------
def test_no_tcq_spec_supplies_bytes_and_that_is_the_design():
    """A spec that ANSWERED for bytes could only be plausible and WRONG.
    ...
    the alphabet directory the manifest declares.  So ``get_format``
    parse-resolves a ``TrellisFormatSpec`` that is exact-or-refuse: it
    answers the activation triple and the capability floor exactly, and it
    RAISES on every byte/bpp helper rather than returning a number.  The
    instance never enters ``REGISTRY`` -- the rate axis is continuous, so
    there is nothing to enumerate -- and ``canonical_format_name`` passes the
    name through unchanged, or every byte path would look up a mangled key.
    """
    ...
    assert "TCQ_E2M1_R640" not in fr.REGISTRY
    assert fr.canonical_format_name("TCQ_E2M1_R640") == "TCQ_E2M1_R640"
    # The refusal is what keeps the closed form out of every byte path.
    with pytest.raises(fr.TrellisSpecFieldRefused):
        spec.memory_bytes_for_shape((1024, 512))
    with pytest.raises(fr.TrellisSpecFieldRefused):
        spec.effective_bits_for_shape((1024, 512))
...
def test_the_ledger_prose_never_quotes_a_count():
    """Entries leave as proof lands; a hardcoded count goes stale immediately."""
...
def test_promotion_ranks_a_trellis_rung_by_its_exact_serialized_rate():
    ...
    from prismaquant.allocator import extend_format_rank_from_candidates

    stats = {
        UNIT_A: {"n_params": 1024 * 512,
                 "out_features": 1024, "in_features": 512},
        UNIT_B: {"n_params": 256 * 512,
                 "out_features": 256, "in_features": 512},
    }
    ...
    rank = extend_format_rank_from_candidates(
        {"NVFP4": 0, "BF16": 1},
        {"NVFP4": 4.5, "BF16": 16.0},
        cands,
        stats,
...
def test_promotion_names_the_rank_table_when_a_format_is_unranked():
    ...
    from prismaquant.allocator_solver import FormatRankUnknownError
    ...
    message = str(exc.value)
    assert "TCQ_E4M3_R1024" in message
    assert UNIT_A in message and UNIT_B in message
    assert "rank table" in message
...
def test_every_entry_that_cites_a_line_range_still_points_at_its_code():
    """Each live citation must still contain the mechanism its reason names."""

    owned = {
        "allocator_candidates.py": "_super_menu_format_names",
    ...
    for site, _why in tm.UNWIRED_LINKS:
        ...
        checked += 1
    assert checked == len(tm.UNWIRED_LINKS), (
        "every live ledger entry must be covered by its citation check"
    )
