...
* **The dev pin's identity check.**  ``load_tessera_contract`` refuses a
  ...
  build is pinned, and it keeps working when the pin is bumped.  (When this was
  written it was also the only way it ran at all: the pin refused the installed
  contract on its recorded sha, so the attested path was dead on this box until
  someone bumped a constant.  It no longer refuses on bytes -- the pin gates on
  the contract's ANSWER now, issue #38 -- but the one-read supply stays,
  because the reason for it was never the pin's staleness.)
