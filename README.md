...
Serving is the separately released [`gridbook`](https://github.com/RobTand/gridbook) package: an out-of-tree vLLM quantization plugin (registry key `gridbook`, legacy read alias `prismaquant`) with zero vLLM-core patches. The released PrismaQuant integration boundary remains Gridbook **0.8.11** at exact commit `187c7216b9d4882321c1923de0b4c49dc139743c`; producer and serving pins are kept in lockstep, and the serving pin additionally binds the reviewed wheel digest. That runtime JIT-builds its native CUDA support kernels and CUTLASS GEMM/grouped-GEMM kernels at model load, attests every required operation and shape before the first forward, and fails closed if the native path is unavailable. It has no Gridbook Triton serving fallback and bypasses vLLM's fallback-capable convenience wrappers. Export is `EXPORT_CONTAINER=nvfp4_cb` (`prismaquant/export_nvfp4_cb.py`). PrismaQuant contains no copy of the runtime: compatibility crosses the repository boundary only through immutable pins and Gridbook's packaged `runtime_contract.json`. This development line can also consume an explicitly supplied Gridbook v11 candidate contract for the RTX 4090 and SM120 validation-only profiles, but those compile-only, unpinned profiles do not alter the released 0.8.11 boundary and do not claim device qualification or release support.
