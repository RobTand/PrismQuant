...
- **Grouped-BMM Fisher: DSv4's `attn.wo_a` is now an allocator decision**
  ...
  and `specs/deepseek_v4.json` byte-for-byte; all three were re-frozen with
  Rob's sign-off in the same merge, and `base.py` was re-frozen once more when
  `origin/main` merged in. One closure file is still UNSTAMPED and the handoff
  verification refuses on it: `production_weight_cache.py`, drifted by the
  unit-sharded render below, needs a reviewed re-freeze.
